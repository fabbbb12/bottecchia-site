...
def test_matches_pure_funding_when_basis_constant():
    ...
    spot = _make_spot([100.0] * 10, funding.index)  # basis sempre zero -> sem efeito extra
    result = run_backtest_f1_realistic(funding, spot, starting_cash=10_000.0, entry_fee_rate=0.0)
...
def test_widening_basis_hurts_short_perp_leg():
    ...
    funding["mark_price"] = [100.0, 105.0, 110.0]  # perpétuo sobe MAIS que o à vista
    spot = _make_spot([100.0, 100.0, 100.0], funding.index)  # à vista parado
    result = run_backtest_f1_realistic(funding, spot, starting_cash=10_000.0, entry_fee_rate=0.0)
...
def test_narrowing_basis_helps_short_perp_leg():
    funding = _make_funding([0.0] * 3)
    funding["mark_price"] = [110.0, 105.0, 100.0]  # perpétuo converge de volta pro à vista
    spot = _make_spot([100.0, 100.0, 100.0], funding.index)
    result = run_backtest_f1_realistic(funding, spot, starting_cash=10_000.0, entry_fee_rate=0.0)
    assert result["final_capital"] > 10_000.0
...
def test_entry_fee_applied_once():
    funding = _make_funding([0.0] * 3)
    spot = _make_spot([100.0] * 3, funding.index)
    result = run_backtest_f1_realistic(funding, spot, starting_cash=10_000.0, entry_fee_rate=0.001)
